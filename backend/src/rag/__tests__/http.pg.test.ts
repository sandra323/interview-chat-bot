...
import { resetChatStoreForTests, getChatStore } from '../../store/chatStore.js';
...
describe.skipIf(!testDatabaseUrl)('KB + documents HTTP with PostgreSQL', () => {
  ...
  it('rejects binding another owner knowledge base to a conversation', async () => {
    const token = await login();
    const mine = await createKnowledgeBase('demo', '我的库');
    const theirs = await createKnowledgeBase('bob', '别人的库');
    const conversationId = getChatStore().createConversation();
    getChatStore().appendMessage(conversationId, 'user', 'hello');

    const stolen = await authJson(
      token,
      'PATCH',
      `/api/conversations/${conversationId}`,
      { knowledgeBaseId: theirs.id },
    );
    expect(stolen.status).toBe(404);
    expect(stolen.body.msg).toMatch(/知识库/);
    expect(getChatStore().getConversationKnowledgeBaseId(conversationId)).toBeNull();

    const ok = await authJson(
      token,
      'PATCH',
      `/api/conversations/${conversationId}`,
