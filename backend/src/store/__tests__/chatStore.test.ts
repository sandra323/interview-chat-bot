...
describe('ChatStore', () => {
  ...
  it('lists conversations with title from first user message', async () => {
    ...
    expect(list[0].knowledgeBaseId).toBeNull();
  ...
  it('binds and unbinds a knowledge base on a conversation', () => {
    const store = createStore();
    const conversationId = store.createConversation();
    const kbId = 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa';
    expect(store.getConversationKnowledgeBaseId(conversationId)).toBeNull();
    expect(store.setConversationKnowledgeBaseId(conversationId, kbId)).toBe(
      true,
    );
    expect(store.getConversationKnowledgeBaseId(conversationId)).toBe(kbId);
    expect(store.setConversationKnowledgeBaseId(conversationId, null)).toBe(
      true,
    );
    expect(store.getConversationKnowledgeBaseId(conversationId)).toBeNull();
    expect(store.setConversationKnowledgeBaseId('missing', kbId)).toBe(false);
    expect(store.setConversationKnowledgeBaseId(conversationId, 'bad')).toBe(
      false,
    );
    store.appendMessage(conversationId, 'user', 'hello');
    store.setConversationKnowledgeBaseId(conversationId, kbId);
    expect(store.listConversations()[0]?.knowledgeBaseId).toBe(kbId);
